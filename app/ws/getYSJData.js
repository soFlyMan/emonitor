...
module.exports = function getQNMData() {
    ...
    axios({
        ...
        .then(response => {
            ...
            MAXID = NMAXID
            console.log('获取到<云神价>第一次maxid', NMAXID)
            // //更新及推送
            setInterval(async function () {
                ...
                await axios({
                    ...
                    .then(response => {
                        ...
                        console.log('持续获取<云神价>maxid中...', 'NMAXID:', NMAXID, 'MAXID:', MAXID)
                        if(l > 0) {
                            NMAXID = parseInt(response.data[l - 1].id)
                        }
                        if (NMAXID !== MAXID) {
                            //获取数据
                            console.log('<云神价>maxid更新，推送商品信息...', 'maxid: ', response.data[l - 1].id, '获取数量:', l)
